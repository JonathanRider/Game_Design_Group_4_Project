...
void BlockVisionComponent::moveTo(sf::Vector2f xy){
  ...
  this->setTopLeft(sf::Vector2f(xy.x - width/2, xy.y - height/2));
  this->setTopRight(sf::Vector2f(xy.x + width/2, xy.y - height/2));
  this->setBottomLeft(sf::Vector2f(xy.x - width/2, xy.y + height/2));
  this->setBottomRight(sf::Vector2f(xy.x + width/2, xy.y + height/2));
}
